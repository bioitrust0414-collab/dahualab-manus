...
function ParkingSection() {
  return <section className="parking-section"><div className="content-shell parking-strip"><div className="parking-photo"><img src={assets.parking} alt="大華醫事檢驗所貴賓專用停車位" /></div><div className="parking-strip-copy"><p className="eyebrow">04 / VISIT WITH EASE</p><h2>看診當天，<em>先解決停車這件事。</em></h2><p>大華醫事檢驗所備有貴賓專用停車位，不用繞路尋找車位、不用擔心違停，讓你把心力留給檢查本身。</p><div className="parking-badge"><Car size={17} /><span>貴賓專用停車位・VIP PARKING ONLY</span></div></div></div></section>;
}

function Booking() {
  const [form, setForm] = useState({ name: "", phone: "", package: "", note: "" });
  const [stage, setStage] = useState<"idle" | "ready" | "fallback">("idle");
  const lineId = "@932cczax";
  const lineUrl = `https://line.me/R/oaMessage/${lineId}/?text=`;
  const lineAddFriendUrl = `https://line.me/R/ti/p/${lineId}`;
  const isMobile = typeof navigator !== "undefined" && /Android|iPhone|iPad|iPod/i.test(navigator.userAgent);
  const message = `【大華醫事檢驗所 預約諮詢】\n姓名：${form.name}\n電話：${form.phone}\n諮詢方案：${form.package}\n備註：${form.note}`;
  const openLine = () => {
    if (!isMobile) { setStage("fallback"); return; }
    const popup = window.open(`${lineUrl}${encodeURIComponent(message)}`, "_blank", "noopener,noreferrer");
    setStage(popup ? "ready" : "fallback");
  };
  const copyMessage = async () => { await navigator.clipboard?.writeText(message); setStage("ready"); };
  return <section className="booking-section" id="booking"><div className="content-shell booking-grid"><div className="booking-copy"><p className="eyebrow">05 / CONTACT & BOOKING</p><h2>把你的問題，<em>帶來聊聊。</em></h2><p>不用註冊，不用先猜方案。留下基本聯絡方式與想了解的方向，我們會把內容整理進 LINE 對話，請你確認後再按下傳送。</p><div className="contact-lines"><a href="tel:047616801"><Phone size={17} /><span><small>諮詢專線</small>04-7616801</span></a><a href="https://maps.google.com/?q=彰化市崙平南路532號" target="_blank" rel="noreferrer"><MapPin size={17} /><span><small>地址</small>彰化市崙平南路 532 號</span></a><a href="https://line.me/ti/p/@932cczax" target="_blank" rel="noreferrer"><MessageCircle size={17} /><span><small>LINE 官方帳號</small>@932cczax</span></a></div></div><div className="booking-card">{stage === "idle" || stage === "fallback" ? <form onSubmit={(event) => { event.preventDefault(); openLine(); }}><div className="form-header"><span>CONSULTATION FORM</span><small>01 — 先留下想了解的事</small></div><div className="form-row"><label>你的稱呼<input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="姓名或單位名稱" autoComplete="name" /></label><label>聯絡電話<input required type="tel" value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} placeholder="方便聯絡的電話" autoComplete="tel" /></label></div><label>想了解的方案<select required value={form.package} onChange={(event) => setForm({ ...form, package: event.target.value })}><option value="">請選擇一個方向</option>{packages.map((item) => <option value={item.code + " " + item.name} key={item.code}>{item.code}｜{item.name}</option>)}<option value="其他 / 不確定">其他／不確定</option></select></label><label>想先問的事情<textarea required value={form.note} onChange={(event) => setForm({ ...form, note: event.target.value })} placeholder="例如：想了解適合我的健檢方向……" rows={4} /></label><button className="button button-primary form-submit" type="submit">{stage === "fallback" ? (isMobile ? "重新開啟 LINE" : "已顯示加入方式") : "整理內容並開啟 LINE"}<ArrowRight size={16} /></button>{stage === "fallback" && <div className="fallback-note">{isMobile ? <><strong>LINE 沒有自動開啟。</strong><span>可以改用複製內容，再貼到 LINE 官方帳號。</span><button type="button" onClick={copyMessage}><Copy size={14} />複製預約內容</button></> : <><strong>電腦版無法直接開啟 LINE App。</strong><span>請用手機掃描下方 QR code 加入官方帳號，或先複製內容，用手機貼到 LINE 傳送給我們。</span><img className="fallback-qr" src={`https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(lineAddFriendUrl)}`} alt="掃描加入大華醫事檢驗所 LINE 官方帳號" width={160} height={160} /><button type="button" onClick={copyMessage}><Copy size={14} />複製預約內容</button></>}</div>}</form> : <div className="booking-success"><div className="success-mark"><Check /></div><p className="eyebrow">MESSAGE READY</p><h3>內容已準備好。</h3><p>LINE 對話已開啟的話，請確認內容後按下傳送；只有完成這一步，我們才會收到你的訊息。</p><div><button className="button button-primary" type="button" onClick={openLine}>重新開啟 LINE <ExternalLink size={15} /></button><button className="button button-quiet" type="button" onClick={copyMessage}><Copy size={15} />複製內容</button></div><button className="reset-link" type="button" onClick={() => { setStage("idle"); setForm({ name: "", phone: "", package: "", note: "" }); }}>重新填寫</button></div>}</div></div></section>;
}
...
export default function Home() {
  return <div className="site"><SiteNav /><main><Hero /><Services /><Packages /><GeneSection /><ParkingSection /><Booking /></main><Footer /><SocialLinks /></div>;
}
